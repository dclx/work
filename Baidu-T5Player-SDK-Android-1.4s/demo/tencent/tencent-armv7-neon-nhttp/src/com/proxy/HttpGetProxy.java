...
public class HttpGetProxy{
	 ...
	 */
	public HttpGetProxy(Context context, String dirPath,int size,int maximum) {
		mContext = context;
		try {
			//初始化代理服务器
	...
	public void startDownload(String id,String url,boolean isDownload) throws Exception {
		...
		if(tmpFile.exists() && tmpFile.length()>=mBufferSize){
			...
			Log.i(TAG, "----not exists:" + mMediaFilePath);
		}
		
		stopDownload();
		if (isDownload) {
			downloadThread = new DownloadThread(mContext, mUrl, mMediaFilePath,mBufferSize);

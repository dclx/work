...
public class DownloadThread extends Thread {
	...
	public DownloadThread(Context context, String url, String savePath,int targetSize) {
		mContext = context;
		mUrl = url;
		mPath = savePath;
	...
	private void download() {
		...
		try {
			...
			while (!mStop //未强制停止
				...
				Log.i(TAG,"--- mDownloadSize :"+mDownloadSize);
				Log.i(TAG,"--- mTargetSize :"+mTargetSize);
				Log.i(TAG,"--- len :"+len);
			}
			if (len == -1) {
				Log.i(TAG,"--- len :"+len);
			...
			Intent intent = new Intent(VideoViewPlayingActivity.SPECIAL_INTENT_FILTER);
			intent.putExtra("mDownloadSize", mDownloadSize);
			intent.putExtra("mTargetSize", mTargetSize);
